...
Reuses the proven pattern from air-langchain-trust, adapted for
...
class AuditChain:
    ...
    def __init__(self, config: AuditConfig | None = None) -> None:
        ...
        if self.config.hmac_secret:
            self._secret = self.config.hmac_secret.encode()
        else:
            key_path = self.config.local_path.replace(".json", "") + ".key"
    ...
    def log_event(
        ...
        *,
        ...
        """Append an event to the audit chain. Returns the signed entry."""
        self._sequence += 1
    ...
    def verify(self) -> ChainVerification:
        ...
        for entry in self._entries:
            ...
            sig_payload = f"{entry.sequence}|{entry.id}|{entry.hash}|{entry.prev_hash}"
